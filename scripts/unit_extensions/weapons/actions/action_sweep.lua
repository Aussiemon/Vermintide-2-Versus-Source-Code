...
ActionSweep.check_precision_target = function (self, owner_unit, owner_player, dedicated_target_range, check_distance, weapon_furthest_point)
	...
	if not HEALTH_ALIVE[current_target] then
		return nil
	end
	...
	if dot < 0.9 or dedicated_target_range < player_to_target_distance then
		good_target = false
	elseif HEALTH_ALIVE[current_target] then
		good_target = true
	end
...
ActionSweep.client_owner_start_action = function (self, new_action, t, chain_action_data, power_level, action_init_data)
	...
	if self._overridable_settings.use_precision_sweep then
		first_person_extension:disable_rig_movement()
...
ActionSweep._get_target_hit_mass = function (self, difficulty_rank, shield_blocked, current_action, breed, hit_unit_id, hit_unit)
	...
	if target_buff_extension then
		hit_mass_total = target_buff_extension:apply_buffs_to_value(hit_mass_total, "hit_mass_amount")
	end
...
ActionSweep._calculate_hit_mass = function (self, difficulty_rank, actual_hit_target_index, shield_blocked, current_action, breed, hit_unit_id, hit_unit)
...
ActionSweep._calculate_hit_mass_level_object = function (self, unit, actual_hit_target_index, current_action)
...
ActionSweep._calculate_attack_direction = function (self, action, weapon_rotation)
	...
	return self._overridable_settings.invert_attack_direction and -attack_direction or attack_direction
end
...
ActionSweep._do_overlap = function (self, dt, t, unit, owner_unit, current_action, physics_world, is_within_damage_window, current_position, current_rotation)
	...
	local attack_direction = self:_calculate_attack_direction(current_action, weapon_rot)
			...
			if is_character and not is_friendly_fire and not hit_self and in_view and (has_hit_precision_target_and_has_last_hit_result or self._hit_units[hit_unit] == nil) then
				...
				if self._overridable_settings.use_precision_sweep and self._precision_target_unit ~= nil and not self._has_hit_precision_target and not final_frame then
					if hit_unit == self._precision_target_unit then
						self._has_hit_precision_target = true
						actual_hit_target_index, shield_blocked, can_damage, can_stagger = self:_calculate_hit_mass(difficulty_rank, actual_hit_target_index, shield_blocked, current_action, breed, hit_unit_id, hit_unit)
				...
				if target_settings then
					...
					if breed then
						...
						hit_armor = HEALTH_ALIVE[hit_unit] and (breed.armor_category == 2 or breed.stagger_armor_category == 2) or breed.armor_category == 3
					else
						hit_zone_name = "torso"
					end
					...
					if shield_blocked or hero_blocking then
						...
						Managers.state.blood:add_enemy_blood(blood_position, hit_unit)
					end
			...
			elseif not is_character and in_view then
				...
				elseif hit_units[hit_unit] == nil and ScriptUnit.has_extension(hit_unit, "health_system") then
					...
					if is_level_unit then
						self:hit_level_object(hit_units, hit_unit, owner_unit, current_action, hit_position, attack_direction, level_index)
...
ActionSweep._play_character_impact = function (self, is_server, attacker_unit, hit_unit, breed, hit_position, hit_zone_name, current_action, damage_profile, target_index, power_level, attack_direction, blocking, boost_curve_multiplier, is_critical_strike, backstab_multiplier)
	...
	local sound_event = no_damage and current_action.stagger_impact_sound_event or self._overridable_settings.impact_sound_event
	...
	if additional_hit_effects then
	...
	if (multiplier_type == "headshot" or multiplier_type == "weakspot" and not blocking) and not current_action.no_headshot_sound and HEALTH_ALIVE[hit_unit] then
		local first_person_extension = ScriptUnit.extension(owner_unit, "first_person_system")
...
ActionSweep._get_damage_profile_name = function (self, action_hand, action)
	return action_hand and action["damage_profile_" .. action_hand] or self._overridable_settings.damage_profile or "default"
end

ActionSweep._populate_sweep_action_data = function (self, action, overrides)
	local overridable_settings = self._overridable_settings
