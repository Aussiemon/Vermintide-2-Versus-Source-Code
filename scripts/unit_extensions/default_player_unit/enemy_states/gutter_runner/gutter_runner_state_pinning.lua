...
GutterRunnerStatePinning.pounce_down = function (self, unit, target_unit, t)
	...
	local towards_foe_rotation = Quaternion.multiply(Quaternion.axis_angle(Vector3.up(), math.pi), flat_unit_rot)
	...
	if Managers.state.network.is_server then
		network_manager.network_transmit:send_rpc_clients("rpc_teleport_unit_to", unit_id, mover_position, towards_foe_rotation)
	else
		network_manager.network_transmit:send_rpc_server("rpc_teleport_unit_to", unit_id, mover_position, towards_foe_rotation)
	end
