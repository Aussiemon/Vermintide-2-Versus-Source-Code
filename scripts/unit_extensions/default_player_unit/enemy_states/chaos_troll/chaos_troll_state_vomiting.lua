...
ChaosTrollStateVomiting.update = function (self, unit, input, dt, context, t)
	...
	elseif self._state == "priming" then
		local input_cancel_vomit = input_extension:get("action_two")
		...
		local input_vomit_released = not input_extension:get("action_career_hold")
...
ChaosTrollStateVomiting._sweep_trajectory_for_heroes = function (self)
	...
	for i = 1, #sweep_positions - 1 do
		...
		local result = PhysicsWorld.linear_sphere_sweep(physics_world, from_pos, to_pos, radius, max_hits, "collision_filter", "filter_enemy_ray_projectile")
		local num_results = result and #result or 0
