...
EndViewStateScoreVSTabReport._set_hero_progression = function (self)
	...
	for i = start_level, self._current_level do
...
EndViewStateScoreVSTabReport._populate_hero_progression = function (self)
	...
	local frame_settings_name = player_data.frame_name
	local level_start = self._level_start
	local start_level, start_experience, start_experience_pool = level_start[1], level_start[2], level_start[3]
...
EndViewStateScoreVSTabReport._setup_entry_animations = function (self, entries, total_experience_gained)
	...
	if num_entries > 0 then
		...
		self:_start_transition_animation("animate_progression_entry_1", "animate_progression_entry", data)
		...
		for i = 0, num_levels_gained do
			...
			elseif i < num_levels_gained then
				...
				self._animation_callbacks[callback_key] = callback(self, "_start_transition_animation", animation_key, "animate_level_up_linear", data)
			else
				local data = {
					starting_progress = 0,
					final_progress = end_experience_level_progress,
				...
				self._animation_callbacks[callback_key] = callback(self, "_start_transition_animation", animation_key, "animate_level_up_end", data)
			end

			callback_key = animation_key
		end

		self._animation_callbacks[callback_key] = callback(self, "_start_transition_animation", "animate_hero_progress", "animate_hero_progress")
