...
DebugManager.update_time_scale = function (self, dt)
	if not not self._disable_time_travel ~= not not script_data.disable_time_travel then
		...
		self:set_time_scale(table.index_of(time_scale_list, 100))
	end
...
DebugManager.propagate_debug_option = function (self, option_hash, cs_index, option_index, dont_save)
	Managers.state.network.network_transmit:send_rpc_server("rpc_propagate_debug_option", option_hash, cs_index, option_index, dont_save)
end

DebugManager.rpc_propagate_debug_option = function (self, channel_id, option_hash_string, cs_index, option_index, dont_save)
	...
	local error_msg = DebugScreen.handle_propagated_option(option_hash, cs_index, option_index, dont_save)
