...
ImguiUILiveCode._merge_changes = function (self, package_name, file_str)
	...
	if compiles then
		local old_data = table.shallow_copy(self._require_datas[package_name])

		self:_mark_package_dirty(package_name, old_data)
		table.merge(self._require_datas[package_name], new_data)

		for k, v in pairs(old_data) do
			if not new_data[k] then
				self._require_datas[package_name][k] = nil
			end
		end
...
ImguiUILiveCode._revert_dirty_packages = function (self, err)
	for package_name, backup_data in pairs(self._dirty_packages) do
		local faulty_data = table.shallow_copy(self._require_datas[package_name])

		table.merge(self._require_datas[package_name], backup_data)

		for k, v in pairs(faulty_data) do
			if not backup_data[k] then
				self._require_datas[package_name][k] = nil
			end
		end
